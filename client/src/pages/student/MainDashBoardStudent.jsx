...
import backImg from '../../assets/images/image10.png';
...
export default function MainDashboardStudent() {
    ...
    useEffect(() => {
        const storedData = localStorage.getItem('ratingSubmission');
        if (storedData) {
            const { submissionDate, userId: storedUserId } = JSON.parse(storedData);
            const oneMonth = 30 * 24 * 60 * 60 * 1000;
            const now = new Date().getTime();

            if (storedUserId === userId && now - new Date(submissionDate).getTime() < oneMonth) {
                setIsDisabled(true);
            }
        }
    }, [userId]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                localStorage.setItem('ratingSubmission', JSON.stringify(submissionData));
                setIsDisabled(true);
                setIsSubmitted(true);
